app.component('designationList', {
    templateUrl: designation_list_template_url,
    controller: function($http, $location, HelperService, $scope, $routeParams, $rootScope, $location, $mdSelect, $element) {
        ...
        var dataTable = $('#designations_list').DataTable({
            ...
            stateLoadCallback: function(settings) {
                ...
                if (state_save_val) {
                    $('#search_designation').val(state_save_val.search.search);
                }
            ...
            scrollCollapse: true,
            ajax: {
                url: laravel_routes['getDesignationList'],
                type: "GET",
                dataType: "json",
                data: function(d) {
                    d.name = $('#designation_name').val();
                    d.status = $('#status').val();
                },
            },
        ...
        $('.refresh_table').on("click", function() {
            $('#designations_list').DataTable().ajax.reload();
        });

        $scope.clear_search = function() {
            $('#search_designation').val('');
            $('#designations_list').DataTable().search('').draw();
        }

        var dataTables = $('#designations_list').dataTable();
...
//------------------------------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------------------------------------
app.component('designationForm', {
    templateUrl: designation_form_template_url,
    controller: function($http, $location, HelperService, $scope, $routeParams, $rootScope) {
        var self = this;
        self.hasPermission = HelperService.hasPermission;
        if (!self.hasPermission('add-designation') || !self.hasPermission('edit-designation')) {
            window.location = "#!/page-permission-denied";
            return false;
        }
        self.angular_routes = angular_routes;
        $http.get(
            laravel_routes['getDesignationFormData'], {
                params: {
            ...
            $rootScope.loading = false;
            if (self.action == 'Edit') {
                if (self.designation.deleted_at) {
